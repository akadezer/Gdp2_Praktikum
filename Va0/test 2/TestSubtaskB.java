...
public class TestSubtaskB {
	...
	public void test_storeTags() {
		...
		assertEquals("Incorrect value for title!", "TANOM Part I: Awakening", tf.getTitle());
		assertEquals("Incorrect value for author!", "Wellenmeister", tf.getAuthor());
		assertEquals("Incorrect value for album!", "TheAbsoluteNecessityOfMeaning", tf.getAlbum());
		assertEquals("Incorrect value for duration!", 355154000L, tf.getDuration());
	}
