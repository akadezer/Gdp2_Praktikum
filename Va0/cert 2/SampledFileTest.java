...
public class SampledFileTest {
	...
	public void testSuperClass() {
		assertEquals("SampledFile is not derived from AudioFile", "AudioFile", clazz.getSuperclass().getName());
	}

	@Test
	public void testAbstract() {
		int mod = clazz.getModifiers();
		assertTrue("SampledFile is not declared abstract", Modifier.isAbstract(mod));
	}
	...
	public void test_timeFormatter_01() {
		assertEquals("Wrong time format", "00:00", SampledFile.timeFormatter(999999L));
		assertEquals("Wrong time format", "00:01", SampledFile.timeFormatter(1000000L));
		assertEquals("Wrong time format", "01:01", SampledFile.timeFormatter(61000000L));
		assertEquals("Wrong time format", "99:00", SampledFile.timeFormatter(5940000000L));
		assertEquals("Wrong time format", "99:59", SampledFile.timeFormatter(5999000000L));
		assertEquals("Wrong time format", "99:59", SampledFile.timeFormatter(5999999999L));
		assertEquals("Wrong time format", "05:05", SampledFile.timeFormatter(305862000L));
		assertEquals("Wrong time format", "05:06", SampledFile.timeFormatter(306862000L));
		assertEquals("Wrong time format", "02:21", SampledFile.timeFormatter(141400816L));
	}
	...
	public void test_getFormattedDuration_01() {
		assertEquals("Wrong formatted duration", "05:31", f1.formatDuration());
		assertEquals("Wrong formatted duration", "02:21", f2.formatDuration());
		assertEquals("Wrong formatted duration", "05:55", f3.formatDuration());
	}
	...
	public void testPlayPauseStop() {
		...
		assertTrue("Position does not change during playback: " + posPlay + " !< " + posPause1, posPlay < posPause1);
		...
		assertEquals("Position in paused state is not stable:" + posPause1 + " != " + posPause2, posPause1, posPause2);
		...
		assertTrue("Position does not change after resuming playback: " + posPause2 + " !< " + posResume1,
				posPause2 < posResume1);
		...
		assertEquals("Position is not stable in state stop: " + posStop1 + " != " + posStop2, posStop1, posStop2);
